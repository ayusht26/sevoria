...
export function TrustStrip() {
  return (
    <section className="bg-canvas border-b border-hairline py-[var(--space-xl)] px-0 overflow-hidden">
      <div className="mx-auto flex w-full max-w-[1200px] flex-col items-center gap-4 text-center px-5 sm:px-6">
        <motion.span
      ...
      {/* Hospital Logos Horizontal Marquee — 2x duplication for seamless loop */}
            ...
            {[...Array(2)].map((_, setIndex) =>
              HOSPITAL_LOGOS.map((item, i) => {
                ...
                return (
                  <div
                    key={`${setIndex}-${i}`}
                    className="px-8 py-3.5 shrink-0 flex items-center justify-center h-14 w-[200px] sm:w-[220px] rounded-xl border border-hairline bg-canvas-elevated"
                    aria-label={item.name}
                  >
                    <LogoComp className="h-7 w-auto opacity-60 md:hover:opacity-100 transition-opacity" />
                  </div>
                )
              })
            )}
